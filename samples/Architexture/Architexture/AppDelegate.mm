...
{
    self.mainNavigationViewController = nil;
    
    
    _m_CheckOutInfo = [[CheckoutInfo alloc]init];
    
    UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"MainStoryboard" bundle: nil];
    Menu_VC* vc = (Menu_VC*)[storyboard instantiateViewControllerWithIdentifier:@"MenuViewController"];
...
- (void) setNavigationViewController:(MainNavController *)navigationViewController
{
...
-(void)showSideMenu
{
    ...
    CGSize viewSize = self.mainNavigationViewController.view.bounds.size;
    UIGraphicsBeginImageContextWithOptions(viewSize, NO, 1.0);
    [self.mainNavigationViewController.view.layer renderInContext:UIGraphicsGetCurrentContext()];
    
    // Read the UIImage object
    UIImage *image = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    
    float sizeStatus = [UIApplication sharedApplication].statusBarFrame.size.height;
    
    // pass this image off to the MenuViewController then swap it in as the rootViewController
    CGRect rect = CGRectMake(0, sizeStatus, viewSize.width, viewSize.height);
    self.menuViewController.screenShotImage = [self imageAtRect:rect andImage:image];
    self.window.rootViewController = self.menuViewController;
}


-(UIImage *)imageAtRect:(CGRect)rect andImage:(UIImage*) img
{
